...
public:
	 ...
	 */
	enum AddrModeType {
		NOADDR = 0, RESADDR = 1, SHORTADDR = 2, EXTADDR = 3
	};
	 ...
	 */
	enum KeyIdModeType {
		IMPLICIT = 0, NOKEYSOURCE = 1, SHORTKEYSOURCE = 2, LONGKEYSOURCE = 3
	};
	...
	/**
	 * Creates the binary representation of the Mac Header depending on its components
	 * \author Tobi
	 */
	std::string GetBinaryRepresentation(void);

	/**
	 * Calculates a new MacHeader with the given Binary Representations of a MAC Header.
	 * This method is needed when creating a new Header of a jammed packet in OQPSK-class.
	 * \author Tobi
	 */
	LrWpanMacHeader calculate_MacHeader(std::string macHeader_Binary);
...
//private:  was private, had to change it because of lr-wpan-header
