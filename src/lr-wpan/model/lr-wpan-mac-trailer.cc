...
TypeId LrWpanMacTrailer::GetTypeId(void) {
	static TypeId tid =
			TypeId("ns3::LrWpanMacTrailer").SetParent<Trailer>().SetGroupName(
					"LrWpan").AddConstructor<LrWpanMacTrailer>();
	return tid;
}
...
uint16_t LrWpanMacTrailer::GetFcs(void) const {
	return m_fcs;
}

void LrWpanMacTrailer::SetFcs(Ptr<const Packet> p) {
	if (m_calcFcs) {
...
void LrWpanMacTrailer::SetFcs(uint16_t fcs) {
	m_fcs = fcs;
}
...
void LrWpanMacTrailer::EnableFcs(bool enable) {
	m_calcFcs = enable;
	if (!enable) {
		m_fcs = 0;
	}
...
bool LrWpanMacTrailer::IsFcsEnabled(void) {
	return m_calcFcs;
}

uint16_t LrWpanMacTrailer::GenerateCrc16(uint8_t *data, int length) {
	int i;
