...
const LrWpanPhyDataAndSymbolRates LrWpanPhy::dataSymbolRates[7] = {
		{ 20.0, 20.0 }, { 40.0, 40.0 }, { 250.0, 12.5 }, { 250.0, 50.0 }, {
...
// The index follows LrWpanPhyOption
const LrWpanPhyPpduHeaderSymbolNumber LrWpanPhy::ppduHeaderSymbolNumbers[7] = {
		{ 32.0, 8.0, 8.0, 8.0 }, //last one is JRB
		{ 32.0, 8.0, 8.0, 8.0 }, { 2.0, 1.0, 0.4, 1.0 }, { 6.0, 1.0, 1.6, 1.0 },
		{ 8.0, 2.0, 2.0, 2.0 }, { 8.0, 2.0, 2.0, 2.0 }, { 8.0, 2.0, 2.0, 2.0 } };

bool LrWpanPhy::jrb_enabled = false;
bool LrWpanPhy::early_jrb = true;

TypeId LrWpanPhy::GetTypeId(void) {
	static TypeId tid =
			TypeId("ns3::LrWpanPhy").SetParent<SpectrumPhy>().SetGroupName(
					"LrWpan").AddConstructor<LrWpanPhy>().AddAttribute(
					"TrxTransitionTime",
					"ON/OFF switching time of the transceiver",
					TimeValue(Seconds(0.001792)),
					MakeTimeAccessor(&LrWpanPhy::m_tRxTransitionTime),
					MakeTimeChecker()).AddTraceSource("TrxState",
					"The state of the transceiver",
					MakeTraceSourceAccessor(&LrWpanPhy::m_trxStateLogger),
					"ns3::LrWpanPhy::StateTracedCallback").AddTraceSource(
					"PhyTxBegin", "Trace source indicating a packet has "
...
LrWpanPhy::LrWpanPhy(void) :
	...
	m_txPsd = psdHelper.CreateTxPowerSpectralDensity(
			m_phyPIBAttributes.phyTransmitPower,
	...
	m_disabled = false;
	jrb_enabled = false; //JRB is disabled by default
}

LrWpanPhy::~LrWpanPhy(void) {
}

void LrWpanPhy::DoDispose(void) {
	...
	m_signal = 0;
	m_errorModel = 0;
	m_pdDataIndicationCallback = MakeNullCallback<void, uint32_t, Ptr<Packet>,
			uint8_t>();
	...
	m_plmeEdConfirmCallback = MakeNullCallback<void, LrWpanPhyEnumeration,
			uint8_t>();
	...
	m_plmeSetTRXStateConfirmCallback = MakeNullCallback<void,
			LrWpanPhyEnumeration>();
	m_plmeSetAttributeConfirmCallback = MakeNullCallback<void,
			LrWpanPhyEnumeration, LrWpanPibAttributeIdentifier>();

	SpectrumPhy::DoDispose();
}

Ptr<NetDevice> LrWpanPhy::GetDevice(void) {
	NS_LOG_FUNCTION(this);
	return m_device;
}

Ptr<MobilityModel> LrWpanPhy::GetMobility(void) {
	NS_LOG_FUNCTION(this);
	return m_mobility;
}
...
Ptr<SpectrumChannel> LrWpanPhy::GetChannel(void) {
	NS_LOG_FUNCTION(this);
	return m_channel;
}

Ptr<const SpectrumModel> LrWpanPhy::GetRxSpectrumModel(void) const {
	NS_LOG_FUNCTION(this);
	if (m_txPsd) {
...
Ptr<AntennaModel> LrWpanPhy::GetRxAntenna(void) {
	NS_LOG_FUNCTION(this);
	return m_antenna;
}
...
void LrWpanPhy::StartRx(Ptr<SpectrumSignalParameters> spectrumRxParams) {
	...
	Ptr<LrWpanSpectrumSignalParameters> lrWpanRxParams = DynamicCast<
			LrWpanSpectrumSignalParameters>(spectrumRxParams);
	...
	if (m_trxState == IEEE_802_15_4_PHY_RX_ON && !m_setTRXState.IsRunning()) {
		...
		// SINR.
		NS_LOG_DEBUG(
				this << " receiving packet with power: " << 10 * log10(LrWpanSpectrumValueHelper::TotalAvgPower (lrWpanRxParams->psd, m_phyPIBAttributes.phyCurrentChannel)) + 30 << "dBm");
		...
		*interferenceAndNoise += *m_noise;
		...
		NS_LOG_DEBUG(
				this << " Cannot receive incoming packet. My transceiver is not in RX state");
		m_phyRxDropTrace(p);
...
void LrWpanPhy::CheckInterference(void) {
	// Calculate whether packet was lost.
	LrWpanSpectrumValueHelper psdHelper;
	Ptr<LrWpanSpectrumSignalParameters> currentRxParams =
			m_currentRxPacket.first;
	...
	if (m_trxState == IEEE_802_15_4_PHY_BUSY_RX) {
		...
		if (m_errorModel != 0) {
			...
			*interferenceAndNoise += *m_noise;
			...
			if (LrWpanOqpsk::o_counter == 2) {
				std::string interferedBinary =
						LrWpanOqpsk::calculateInterference();

				if (interferedBinary.length() != 0) { //empty, if no interference
					interfered = true;
					NS_LOG_UNCOND(this  << " !!!INTERFERENCE DETECTED!!!\n");
					NS_LOG_UNCOND(
							this << ": Position of first jammed bit: " << LrWpanOqpsk::o_number_NonJammedBitsStart);
					NS_LOG_UNCOND(
							this << ": First jammed field: " << LrWpanOqpsk::GetFirstJammedComponent((int)LrWpanOqpsk::o_number_NonJammedBitsStart));
					NS_LOG_UNCOND(
							this << ": Number of jammed bits: " << LrWpanOqpsk::o_number_JammedBits);
					NS_LOG_UNCOND(
							this << ": Old packet binary:\n" << LrWpanOqpsk::GetBinaryRepresentation(currentPacket)<<"\n");
					NS_LOG_UNCOND(
							this << ": New packet binary:\n" << interferedBinary<<"\n");
					Ptr<Packet> interferedPacket;
					interferedPacket = LrWpanOqpsk::GetPacket(interferedBinary);

					NS_LOG_UNCOND(
							"-----------------------------------------------------------------");
					NS_LOG_UNCOND(
							this<< " : Interfered Packet Representation: \n");
					interferedPacket->EnablePrinting();
					interferedPacket->Print(std::clog);
					NS_LOG_UNCOND(
							"\n-----------------------------------------------------------------\n");
				} else {
					NS_LOG_DEBUG(this << " No Interference was detected.");
				}
			}
			if (m_random->GetValue() < per || interfered) {
				// The packet was destroyed, drop the packet after reception.
				m_currentRxPacket.second = true;
				NS_LOG_UNCOND(
						"\n" << this << ": The received Packet was destroyed due to interference.\n");
				//-------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
void LrWpanPhy::EndRx(Ptr<SpectrumSignalParameters> par) {
	...
	//For the Jammer: mustn't receive any packet while sending one. Necessary due to the inactivation of CSMA/CA.
	if (m_trxState == IEEE_802_15_4_PHY_TX_ON
			|| m_trxState == IEEE_802_15_4_PHY_BUSY_TX) {
		NS_LOG_DEBUG(
				"\n" << this << ": Cannot receive packet. I am currently transmitting a packet.\n");
		return;
	}

	Ptr<LrWpanSpectrumSignalParameters> params = DynamicCast<
			LrWpanSpectrumSignalParameters>(par);
	...
	if (currentRxParams == params) {
		CheckInterference();
		Ptr<Packet> currentPacket =
				currentRxParams->packetBurst->GetPackets().front(); //get the current packet
		currentPacket->EnablePrinting();
	...
	if (params == 0) {
		NS_LOG_LOGIC(
				"Node: " << m_device->GetAddress() << " Removing interferent: " << *(par->psd));
		return;
	}
	...
	if (currentRxParams == params) {
		...
		if (!m_currentRxPacket.second) {
			...
			if (!m_pdDataIndicationCallback.IsNull()) {
				//The packet is received. Remove the PHY header.
				//------------------------------------------------------------------------------------------------------------------------------------------------------
				NS_LOG_DEBUG(this << ": The packet was successfully received");
				LrWpanPhyHeader phyHeader;
				currentPacket->PeekHeader(phyHeader);
				currentPacket->RemoveHeader(phyHeader);
				//-------------------------------------------------------------------------------------------------------------------------------------------------------
			...
			NS_LOG_UNCOND(this << ": The received packet was destroyed. \n");
		...
		if (m_trxStatePending != IEEE_802_15_4_PHY_IDLE) {
			...
			if (!m_setTRXState.IsRunning()) {
				NS_LOG_LOGIC(
						"Apply pending state change to " << m_trxStatePending);
...
void LrWpanPhy::PdDataRequest(const uint32_t psduLength, Ptr<Packet> p) {
	...
	if (!m_setTRXState.IsRunning()) {
		if (m_trxState == IEEE_802_15_4_PHY_TX_ON) {
			...
			///-----------------------------------------------------------------------------------------------------------------
			//remove Mac Header and add new general header
			p = p->Copy(); //MAC header uses reference to the packet. Has to be seperated between the layers.

//			LrWpanMacHeader macHdr;
//			p->PeekHeader(macHdr);
//			LrWpanHeader header(macHdr);
//			header.SetPhr(psduLength);
//			p->RemoveHeader(macHdr);
//			p->AddHeader(header);
			LrWpanPhyHeader phyHeader(psduLength);
			p->AddHeader(phyHeader);

			///-----------------------------------------------------------------------------------------------------------------

			Ptr<LrWpanSpectrumSignalParameters> txParams = Create<
			...
			///-----------------------------------------------------------------------------------------------------------------
			//let's add the packet to the interference handling class OQPSK
			LrWpanOqpsk::addPacket(p, Simulator::Now(), txParams->duration);
			///-----------------------------------------------------------------------------------------------------------------
			m_pdDataRequest = Simulator::Schedule(txParams->duration,
					&LrWpanPhy::EndTx, this);
			...
			NS_FATAL_ERROR(
					"This should be unreachable, or else state " << m_trxState << " should be added as a case");
		}
	} else {
...
void LrWpanPhy::PlmeCcaRequest(void) {
	NS_LOG_FUNCTION(this);
...
void LrWpanPhy::PlmeEdRequest(void) {
	NS_LOG_FUNCTION(this);
	if (m_trxState == IEEE_802_15_4_PHY_RX_ON
			|| m_trxState == IEEE_802_15_4_PHY_BUSY_RX) {
...
void LrWpanPhy::PlmeSetTRXStateRequest(LrWpanPhyEnumeration state) {
	...
	if (state == IEEE_802_15_4_PHY_TRX_OFF) {
		...
		if ((m_trxState == IEEE_802_15_4_PHY_BUSY_RX)
			...
			m_setTRXState = Simulator::Schedule(m_tRxTransitionTime,
					&LrWpanPhy::EndSetTRXState, this);
	...
	if (state == IEEE_802_15_4_PHY_TX_ON) {
		...
		if ((m_trxState == IEEE_802_15_4_PHY_BUSY_RX)
			...
			Time setTime = Seconds(
					(double) aTurnaroundTime / GetDataOrSymbolRate(false));
			m_setTRXState = Simulator::Schedule(setTime,
					&LrWpanPhy::EndSetTRXState, this);
	...
	if (state == IEEE_802_15_4_PHY_FORCE_TRX_OFF) {
		if (m_trxState == IEEE_802_15_4_PHY_TRX_OFF) {
			...
			if (m_currentRxPacket.first) {   //terminate reception if needed
											 //incomplete reception -- force packet discard
	...
	NS_FATAL_ERROR(
			"Unexpected transition from state " << m_trxState << " to state " << state);
}
...
bool LrWpanPhy::ChannelSupported(uint8_t channel) {
	...
	for (uint32_t i = 0; i < 32; i++) {
		if ((m_phyPIBAttributes.phyChannelsSupported[i] & (1 << channel))
				!= 0) {
			retValue = true;
			break;
		}
...
void LrWpanPhy::PlmeSetAttributeRequest(LrWpanPibAttributeIdentifier id,
	...
	case phyChannelsSupported: { // only the first element is considered in the array
		if ((attribute->phyChannelsSupported[0] & 0xf8000000) != 0) { //5 MSBs reserved
			status = IEEE_802_15_4_PHY_INVALID_PARAMETER;
		} else {
...
void LrWpanPhy::ChangeTrxState(LrWpanPhyEnumeration newState) {
	...
	if (m_trxState == IEEE_802_15_4_PHY_TX_ON) //9
			{
		NotifyListenersTx();
	} else if (m_trxState == IEEE_802_15_4_PHY_BUSY_TX) //2
			{
		NotifyListenersTxStart();
	} else if (m_trxState == IEEE_802_15_4_PHY_RX_ON) //6
			{
		NotifyListenersRx();
	} else if (m_trxState == IEEE_802_15_4_PHY_BUSY_RX) //1
			{
		NotifyListenersRxStart();
	} else if (m_trxState == IEEE_802_15_4_PHY_TRX_OFF) //8
			{
		NotifyListenersSleep();
	}
}

bool LrWpanPhy::PhyIsBusy(void) const {
...
void LrWpanPhy::EndSetTRXState(void) {
	NS_LOG_FUNCTION(this);
...
Time LrWpanPhy::CalculateTxTime(Ptr<const Packet> packet) {
	...
	if (jrb_enabled) {
		txTime += Seconds(
				(packet->GetSize() - 7) * 8.0 / GetDataOrSymbolRate(isData)); //added "-7" because otherwise it would have been doubled since the PHY header is contained in the packet now
	} else {
		txTime += Seconds(
				(packet->GetSize() - 6) * 8.0 / GetDataOrSymbolRate(isData));
	}
	return txTime;
}
...
Time LrWpanPhy::GetPpduHeaderTxTime(void) {
	NS_LOG_FUNCTION(this);
	...
	if (jrb_enabled) {
		totalPpduHdrSymbols += +ppduHeaderSymbolNumbers[m_phyOption].jrb; //If jrb is enabled, the header becomes 8 bits larger
	}
...
void LrWpanPhy::SetMyPhyOption(void) {
	NS_LOG_FUNCTION(this);

	m_phyOption = IEEE_802_15_4_INVALID_PHY_OPTION;

	if (m_phyPIBAttributes.phyCurrentPage == 0) {
		if (m_phyPIBAttributes.phyCurrentChannel == 0) {  // 868 MHz BPSK
			m_phyOption = IEEE_802_15_4_868MHZ_BPSK;
		} else if (m_phyPIBAttributes.phyCurrentChannel <= 10) { // 915 MHz BPSK
			m_phyOption = IEEE_802_15_4_915MHZ_BPSK;
		} else if (m_phyPIBAttributes.phyCurrentChannel <= 26) { // 2.4 GHz MHz O-QPSK
			m_phyOption = IEEE_802_15_4_2_4GHZ_OQPSK;
		}
	} else if (m_phyPIBAttributes.phyCurrentPage == 1) {
		if (m_phyPIBAttributes.phyCurrentChannel == 0) {  // 868 MHz ASK
			m_phyOption = IEEE_802_15_4_868MHZ_ASK;
		} else if (m_phyPIBAttributes.phyCurrentChannel <= 10) {  // 915 MHz ASK
			m_phyOption = IEEE_802_15_4_915MHZ_ASK;
		}
	} else if (m_phyPIBAttributes.phyCurrentPage == 2) {
		if (m_phyPIBAttributes.phyCurrentChannel == 0) {  // 868 MHz O-QPSK
			m_phyOption = IEEE_802_15_4_868MHZ_OQPSK;
		} else if (m_phyPIBAttributes.phyCurrentChannel <= 10) { // 915 MHz O-QPSK
			m_phyOption = IEEE_802_15_4_915MHZ_OQPSK;
		}
	}
}

LrWpanPhyOption LrWpanPhy::GetMyPhyOption(void) {
	NS_LOG_FUNCTION(this);
	return m_phyOption;
}
...
void LrWpanPhy::SetTxPowerSpectralDensity(Ptr<SpectrumValue> txPsd) {
	...
	m_txPsd = txPsd;
	NS_LOG_INFO(
			"\t computed tx_psd: " << *txPsd << "\t stored tx_psd: " << *m_txPsd);
}
...
Ptr<const SpectrumValue> LrWpanPhy::GetNoisePowerSpectralDensity(void) {
	NS_LOG_FUNCTION(this);
	return m_noise;
}
...
Ptr<LrWpanErrorModel> LrWpanPhy::GetErrorModel(void) const {
	NS_LOG_FUNCTION(this);
	return m_errorModel;
}

uint64_t LrWpanPhy::GetPhySHRDuration(void) const {
	NS_LOG_FUNCTION(this);
	NS_ASSERT(m_phyOption < IEEE_802_15_4_INVALID_PHY_OPTION);
...
double LrWpanPhy::GetPhySymbolsPerOctet(void) const {
	NS_LOG_FUNCTION(this);
	NS_ASSERT(m_phyOption < IEEE_802_15_4_INVALID_PHY_OPTION);
...
//+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
/**
 * This section describes the main part of the implementation that is necessary to indroduce a JRB
 * 
 */

/**
 * Enables the physical jamming resistance
 */
void LrWpanPhy::EnableJRB() {
	jrb_enabled = true;
}

/**
 * Disables the physical jamming resistance
 */
void LrWpanPhy::DisableJRB() {
	jrb_enabled = false;
}

bool LrWpanPhy::IsJrbEnabled() {
	return jrb_enabled;
}

void LrWpanPhy::SetEarlyJRB(bool earlyJRB) {
	early_jrb = earlyJRB;
}

bool LrWpanPhy::IsEarlyJRB() {
	return early_jrb;
}

/**
 * Describes the shape of the PHY Header
 * @return the Header To String
 */
std::string LrWpanPhy::ToString() const {

	std::ostringstream oss;
	Print(oss);
	return oss.str();
}
void LrWpanPhy::Print(std::ostream &os) const {
	os << "SHRPreamble: " << ppduHeaderSymbolNumbers[m_phyOption].shrPreamble
			<< " , StartFrameDelimiter: "
			<< ppduHeaderSymbolNumbers[m_phyOption].shrSfd << " , PHR: "
			<< ppduHeaderSymbolNumbers[m_phyOption].phr << " , JRB: "
			<< ppduHeaderSymbolNumbers[m_phyOption].jrb;
}
...
LrWpanPhyEnumeration LrWpanPhy::GetTRXState(void) {
	NS_LOG_FUNCTION(this);
	return m_trxState;
}

Time LrWpanPhy::GetTrxTransitionTime(void) {
	NS_LOG_FUNCTION(this);
	return m_tRxTransitionTime;
}

void LrWpanPhy::EnergyDepletionHandler(void) {
	NS_LOG_FUNCTION(this);
	NS_LOG_DEBUG("Energy depleted at node, stopping rx/tx activities");
	m_disabled = true;
}

void LrWpanPhy::EnergyRechargedHandler(void) {
	NS_LOG_FUNCTION(this);
	NS_LOG_DEBUG("Energy recharged at node, restarting rx/tx activities");
	m_disabled = false;
}

void LrWpanPhy::RegisterListener(LrWpanPhyListener *listener) {
	m_listeners.push_back(listener);
}

void LrWpanPhy::NotifyListenersRx(void) {
...
void LrWpanPhy::NotifyListenersRxStart(void) {
	ListenerList::const_iterator it = m_listeners.begin();
	for (; it != m_listeners.end(); it++) {
		(*it)->NotifyRxStart();
	}
...
void LrWpanPhy::NotifyListenersTx(void) {
	ListenerList::const_iterator it = m_listeners.begin();
	for (; it != m_listeners.end(); it++) {
		(*it)->NotifyTx();
	}
...
void LrWpanPhy::NotifyListenersTxStart(void) {
	ListenerList::const_iterator it = m_listeners.begin();
	for (; it != m_listeners.end(); it++) {
		(*it)->NotifyTxStart();
	}
...
void LrWpanPhy::NotifyListenersSleep(void) {
	ListenerList::const_iterator it = m_listeners.begin();
	for (; it != m_listeners.end(); it++) {
		(*it)->NotifySleep();
	}
...
void LrWpanPhy::NotifyListenersTransition(LrWpanPhyEnumeration nextState) {
	ListenerList::const_iterator it = m_listeners.begin();
	for (; it != m_listeners.end(); it++) {
