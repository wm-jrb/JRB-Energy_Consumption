...
TypeId LrWpanContikiMac::GetTypeId(void) {
	static TypeId tid =
			TypeId("ns3::LrWpanContikiMac").SetParent<LrWpanMac>().SetGroupName(
					"LrWpan").AddConstructor<LrWpanContikiMac>().AddAttribute(
					"SleepTime", "Sleep interval", DoubleValue(0.125),
					MakeDoubleAccessor(&LrWpanContikiMac::m_sleepTime),
					MakeDoubleChecker<double>()).AddAttribute("CcaInterval",
					"The interval between each CCA", TimeValue(Seconds(0.0006)),
					MakeTimeAccessor(&LrWpanContikiMac::m_ccaInterval),
					MakeTimeChecker()).AddAttribute("PktInterval",
					"The interval between each packet transmission",
					TimeValue(Seconds(0.00055)),
					MakeTimeAccessor(&LrWpanContikiMac::m_pktInterval),
					MakeTimeChecker()).AddAttribute("RdcMaxRetries",
					"The maximum number of retries for ContikiMAC RDC",
					UintegerValue(4),
					MakeUintegerAccessor(
							&LrWpanContikiMac::m_rdcMaxFrameRetries),
					MakeUintegerChecker<uint8_t>()).AddAttribute("FastSleep",
					"ContikiMAC fast sleep optimization", BooleanValue(false),
...
void LrWpanContikiMac::DoInitialize() {
	//Sleep
...
void LrWpanContikiMac::DoDispose() {
...
void LrWpanContikiMac::Sleep() {
	NS_LOG_FUNCTION(this);
	NS_ASSERT(m_sleep.IsExpired());
	if (m_lrWpanMacState == MAC_IDLE
			&& (m_phy->GetTRXState() == IEEE_802_15_4_PHY_RX_ON
					|| m_phy->GetTRXState() == IEEE_802_15_4_PHY_TX_ON)) //MAC_IDLE and TRX_ON
			{
		NS_LOG_DEBUG("Sleeping...");
		m_phy->PlmeSetTRXStateRequest(IEEE_802_15_4_PHY_TRX_OFF);
	} else {
		Time sleepTime = maxPacketTime + m_pktInterval * 2;
		m_sleep = Simulator::Schedule(sleepTime, &LrWpanContikiMac::Sleep,
				this);
	}
}

void LrWpanContikiMac::WakeUp() {
	NS_LOG_FUNCTION(this);
...
Time LrWpanContikiMac::CheckPlm(Address address) {
	NS_ASSERT_MSG(
			Mac16Address::IsMatchingType(address)
					|| Mac64Address::IsMatchingType(address),
...
void LrWpanContikiMac::NotifyPlm(Address address, Time txStart) {
	NS_ASSERT_MSG(
			Mac16Address::IsMatchingType(address)
					|| Mac64Address::IsMatchingType(address),
			"Not a valid Address type");
...
void LrWpanContikiMac::McpsDataRequest(McpsDataRequestParams params,
		Ptr<Packet> p) {
	NS_LOG_FUNCTION(this << p);

	NS_ASSERT_MSG(!GetCsmaCa()->IsSlottedCsmaCa(),
			"ContikiMAC cannot be used in slotted mode");
...
bool LrWpanContikiMac::CheckQueue() {
	NS_LOG_FUNCTION(this);
	...
	if (m_lrWpanMacState == MAC_IDLE && !m_txQueue.empty() && m_txPkt == 0
		...
		if (hdr.GetDstAddrMode() == SHORT_ADDR
				&& hdr.GetShortDstAddr() == Mac16Address("ff:ff")) //Broadcast case
						{
			NS_LOG_DEBUG("Broadcast, scheduling packet now");
			m_setMacState = Simulator::ScheduleNow(
					&LrWpanContikiMac::SetLrWpanMacState, this, MAC_CSMA);
		} else if (hdr.GetDstAddrMode() == SHORT_ADDR
				&& CheckPlm(hdr.GetShortDstAddr()) != Seconds(0)) {
			...
			uint32_t t = ceil(d);
			Time txTime = CheckPlm(hdr.GetShortDstAddr())
					+ Seconds(t * m_sleepTime) - Simulator::Now();
			NS_LOG_DEBUG(
					"t = " << t << " d = " << d << " diff = " << diff << " txTime: " << Simulator::Now () + txTime << " PLM: " << CheckPlm (hdr.GetShortDstAddr ()));
			m_setMacState = Simulator::Schedule(txTime,
					&LrWpanContikiMac::SetLrWpanMacState, this, MAC_CSMA);
		} else if (hdr.GetDstAddrMode() == EXT_ADDR
				&& CheckPlm(hdr.GetExtDstAddr()) != Seconds(0)) {
			NS_LOG_DEBUG("Found neighbor in phase-lock module");
			Time diff = Simulator::Now() - CheckPlm(hdr.GetExtDstAddr());
			double d = diff.GetDouble() / m_sleepTime / (1000 * 1000 * 1000);
			uint32_t t = ceil(d);
			Time txTime = CheckPlm(hdr.GetExtDstAddr())
					+ Seconds(t * m_sleepTime) - Simulator::Now();
			NS_LOG_DEBUG(
					"t = " << t << " d = " << d << " diff = " << diff << " txTime: " << Simulator::Now () + txTime << " PLM: " << CheckPlm (hdr.GetExtDstAddr ()));
			m_setMacState = Simulator::Schedule(txTime,
					&LrWpanContikiMac::SetLrWpanMacState, this, MAC_CSMA);
...
void LrWpanContikiMac::PdDataIndication(uint32_t psduLength, Ptr<Packet> p,
		uint8_t lqi) {
	number_received_packets++; 

	NS_ASSERT(
			m_lrWpanMacState == MAC_IDLE || m_lrWpanMacState == MAC_ACK_PENDING
	...
	if (!receivedMacTrailer.CheckFcs(p)) {
		...
		NS_LOG_DEBUG(
				"Packet from " << params.m_srcAddr << " to " << params.m_dstAddr);
		...
		if (m_macPromiscuousMode) {
			...
			if (!m_mcpsDataIndicationCallback.IsNull()) {
				...
				if (m_lrWpanMacState == MAC_IDLE) {
					...
					m_sleep.Cancel();
					m_sleep = Simulator::ScheduleNow(&LrWpanContikiMac::Sleep,
							this);        //Sleep during promiscuous mode?
				}
			} else {
				NS_LOG_ERROR(
						this << " Data Indication Callback not initialised");
			}
		} else {
			//level 3 frame filtering
			acceptFrame = (receivedMacHdr.GetType()
					!= LrWpanMacHeader::LRWPAN_MAC_RESERVED);
			...
			if (acceptFrame
					&& ((receivedMacHdr.GetType()
							== LrWpanMacHeader::LRWPAN_MAC_DATA)
							|| (receivedMacHdr.GetType()
			...
			if (acceptFrame) {
				...
				if ((receivedMacHdr.IsData() || receivedMacHdr.IsCommand())
						...
						&& !(receivedMacHdr.GetDstAddrMode() == SHORT_ADDR
								&& receivedMacHdr.GetShortDstAddr() == "ff:ff")) {
					...
					if (m_lrWpanMacState == MAC_ACK_PENDING) {
						if (m_repeatPkt.IsRunning()) {
							NS_LOG_DEBUG(
									"ContikiMAC pkt retransmission cancelled");
							m_repeatPkt.Cancel();
							m_rdcRetries = 0;
						}
				...
				if (receivedMacHdr.IsData()
					...
					if (m_lrWpanMacState == MAC_IDLE) {
						if (!receivedMacHdr.IsAckReq()
								|| receivedMacHdr.GetShortDstAddr() == "ff:ff") //If Ack not required, go to sleep
										{
							NS_LOG_DEBUG("Fwd Up & Sleep");
							m_sleep.Cancel();
							m_sleep = Simulator::ScheduleNow(
									&LrWpanContikiMac::Sleep, this);
						} else
							m_sleep.Cancel();
					}
					...
					if (receivedMacHdr.GetSeqNum() == macHdr.GetSeqNum()) {
						...
						m_setMacState.Cancel();
						m_setMacState = Simulator::ScheduleNow(
								&LrWpanContikiMac::SetLrWpanMacState, this,
								MAC_IDLE);
					} else {
...
void LrWpanContikiMac::PktRetransmissionTimeout(void) {
	NS_LOG_FUNCTION(this);

	if (m_broadcast) {
		NS_ASSERT(
				m_lrWpanMacState == MAC_SENDING
						&& m_phy->GetTRXState() == IEEE_802_15_4_PHY_TX_ON);
...
void LrWpanContikiMac::PdDataConfirm(LrWpanPhyEnumeration status) {
	...
	if (status == IEEE_802_15_4_PHY_SUCCESS) {
		if (!macHdr.IsAcknowledgment()) {
			...
			if (macHdr.IsAckReq()) {
				...
				m_setMacState = Simulator::ScheduleNow(
						&LrWpanContikiMac::SetLrWpanMacState, this,
						MAC_ACK_PENDING);     //actually RDC Ack Pending

				//ContikiMAC: data packet transmissions are repeated
				NS_ASSERT(m_repeatPkt.IsExpired());
				m_repeatPkt = Simulator::Schedule(m_pktInterval,
						&LrWpanContikiMac::PktRetransmissionTimeout, this);

				return;
			} else if (m_broadcast
					&& ((Simulator::Now() - m_bcStart) < Seconds(m_sleepTime))) {
				NS_ASSERT(m_repeatPkt.IsExpired());
				m_repeatPkt = Simulator::Schedule(m_pktInterval,
						&LrWpanContikiMac::PktRetransmissionTimeout, this);
			} else {
	...
	if (!m_broadcast) {
		...
		m_setMacState.Cancel();
		m_setMacState = Simulator::ScheduleNow(
				&LrWpanContikiMac::SetLrWpanMacState, this, MAC_IDLE);
	}
}

void LrWpanContikiMac::PlmeCcaConfirm(LrWpanPhyEnumeration status) {
	...
	if (m_lrWpanMacState == MAC_IDLE) {
		...
		if (status == IEEE_802_15_4_PHY_IDLE) {
			if (m_ccaCount == 2) {
				...
				Simulator::Schedule(m_ccaInterval, &LrWpanPhy::PlmeCcaRequest,
						m_phy);
			}
...
void LrWpanContikiMac::PlmeSetTRXStateConfirm(LrWpanPhyEnumeration status) {
	NS_LOG_FUNCTION(this << status);
		...
		if (m_phy->GetTRXState() == IEEE_802_15_4_PHY_TRX_OFF) {
			...
			if (Simulator::Now() > m_nextWakeUp) {
				...
				while (upTime <= Seconds(0)) {
					upTime = Seconds(m * m_sleepTime) + m_nextWakeUp
							- Simulator::Now(); //Keep incr. 'm' if missed > 1 wake-ups
					m++;
				}
				NS_LOG_DEBUG(
						"WakeUp Missed! next wake-up: " << Simulator::Now () + upTime << " nWU: " << m_nextWakeUp);
			...
			} else  //will be equal only at start
			{
				...
				NS_LOG_DEBUG(
						"next WakeUp (init): " << Simulator::Now () + upTime);
			}
			m_wakeUp = Simulator::Schedule(upTime, &LrWpanContikiMac::WakeUp,
					this);
...
void LrWpanContikiMac::SetLrWpanMacState(LrWpanMacState macState) {
...
void LrWpanContikiMac::SetLrWpanRadioState(const LrWpanPhyEnumeration state) {
	...
	default:
		NS_FATAL_ERROR(
				"LrWpanRadioEnergyModel:Undefined radio state:" << m_currentState);
	}
	NS_LOG_DEBUG(
			"LrWpanContikiMac:Switching to state: " << stateName << " at time = " << Simulator::Now ());
}
