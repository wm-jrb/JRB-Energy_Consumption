...
TypeId LrWpanCsmaCa::GetTypeId(void) {
	static TypeId tid =
			TypeId("ns3::LrWpanCsmaCa").SetParent<Object>().SetGroupName(
					"LrWpan").AddConstructor<LrWpanCsmaCa>();
	return tid;
}
...
void LrWpanCsmaCa::DoDispose() {
	m_lrWpanMacStateCallback = MakeNullCallback<void, LrWpanMacState>();
	Cancel();
	m_mac = 0;
}
void LrWpanCsmaCa::setCsmaEnabled(bool enabled) {
	m_csma_enabled = enabled;
}

bool LrWpanCsmaCa::getCsmaEnabled() {
	return m_csma_enabled;
}

void LrWpanCsmaCa::SetMac(Ptr<LrWpanMac> mac) {
	m_mac = mac;
}

Ptr<LrWpanMac> LrWpanCsmaCa::GetMac(void) const {
	return m_mac;
}

void LrWpanCsmaCa::SetSlottedCsmaCa(void) {
	NS_LOG_FUNCTION(this);
	m_isSlotted = true;
}

void LrWpanCsmaCa::SetUnSlottedCsmaCa(void) {
	NS_LOG_FUNCTION(this);
	m_isSlotted = false;
}

bool LrWpanCsmaCa::IsSlottedCsmaCa(void) const {
	NS_LOG_FUNCTION(this);
	return (m_isSlotted);
}

bool LrWpanCsmaCa::IsUnSlottedCsmaCa(void) const {
	NS_LOG_FUNCTION(this);
	return (!m_isSlotted);
}

void LrWpanCsmaCa::SetMacMinBE(uint8_t macMinBE) {
	NS_LOG_FUNCTION(this << macMinBE);
	m_macMinBE = macMinBE;
}

uint8_t LrWpanCsmaCa::GetMacMinBE(void) const {
	NS_LOG_FUNCTION(this);
	return m_macMinBE;
}

void LrWpanCsmaCa::SetMacMaxBE(uint8_t macMaxBE) {
	NS_LOG_FUNCTION(this << macMaxBE);
	m_macMinBE = macMaxBE;
}

uint8_t LrWpanCsmaCa::GetMacMaxBE(void) const {
	NS_LOG_FUNCTION(this);
	return m_macMaxBE;
}

void LrWpanCsmaCa::SetMacMaxCSMABackoffs(uint8_t macMaxCSMABackoffs) {
	NS_LOG_FUNCTION(this << macMaxCSMABackoffs);
	m_macMaxCSMABackoffs = macMaxCSMABackoffs;
}

uint8_t LrWpanCsmaCa::GetMacMaxCSMABackoffs(void) const {
	NS_LOG_FUNCTION(this);
	return m_macMaxCSMABackoffs;
}

void LrWpanCsmaCa::SetUnitBackoffPeriod(uint64_t unitBackoffPeriod) {
	NS_LOG_FUNCTION(this << unitBackoffPeriod);
	m_aUnitBackoffPeriod = unitBackoffPeriod;
}

uint64_t LrWpanCsmaCa::GetUnitBackoffPeriod(void) const {
	NS_LOG_FUNCTION(this);
	return m_aUnitBackoffPeriod;
}

Time LrWpanCsmaCa::GetTimeToNextSlot(void) const {
	NS_LOG_FUNCTION(this);
...

{
	...
	if (IsSlottedCsmaCa()) {
		...
		m_randomBackoffEvent = Simulator::Schedule(backoffBoundary,
				&LrWpanCsmaCa::RandomBackoffDelay, this);
	} else {
		m_BE = m_macMinBE;
 ...
 */
void LrWpanCsmaCa::RandomBackoffDelay() {
	NS_LOG_FUNCTION(this);
	...
	randomBackoff = MicroSeconds(
			backoffPeriod * GetUnitBackoffPeriod() * 1000 * 1000 / symbolRate);

	//if CSMA/CA is disabled, we set the random Backoff to zero (for the jammer),
	if (!m_csma_enabled) {
		randomBackoff = MicroSeconds(0);
	}
	if (IsUnSlottedCsmaCa()) {
		NS_LOG_LOGIC(
				"Unslotted:  requesting CCA after backoff of " << randomBackoff.GetMicroSeconds () << " us");
		m_requestCcaEvent = Simulator::Schedule(randomBackoff,
				&LrWpanCsmaCa::RequestCCA, this);
	} else {
		NS_LOG_LOGIC(
				"Slotted:  proceeding after backoff of " << randomBackoff.GetMicroSeconds () << " us");
		m_canProceedEvent = Simulator::Schedule(randomBackoff,
				&LrWpanCsmaCa::CanProceed, this);
	}
}

// TODO : Determine if transmission can be completed before end of CAP for the slotted csmaca
//        If not delay to the next CAP
void LrWpanCsmaCa::CanProceed() {
	NS_LOG_FUNCTION(this);
	...
	if (canProceed) {
		...
		Time backoffBoundary = GetTimeToNextSlot();
		m_requestCcaEvent = Simulator::Schedule(backoffBoundary,
				&LrWpanCsmaCa::RequestCCA, this);
	} else {
		Time nextCap = Seconds(0);
		m_randomBackoffEvent = Simulator::Schedule(nextCap,
				&LrWpanCsmaCa::RandomBackoffDelay, this);
	}
}

void LrWpanCsmaCa::RequestCCA() {
	NS_LOG_FUNCTION(this);
 ...
 */
void LrWpanCsmaCa::PlmeCcaConfirm(LrWpanPhyEnumeration status) {
	...
	if (m_ccaRequestRunning) {
		...
		//if the CSMA/CA is enabled, the channel shall always appear idle
		if (!m_csma_enabled) {
		...
		if (status == IEEE_802_15_4_PHY_IDLE) {
			if (IsSlottedCsmaCa()) {
				...
				if (m_CW == 0) {
					...
					m_requestCcaEvent = Simulator::ScheduleNow(
							&LrWpanCsmaCa::RequestCCA, this); // Perform CCA again
				}
			} else {
				// inform MAC, channel is idle
				if (!m_lrWpanMacStateCallback.IsNull()) {
			...
			m_BE = std::min(static_cast<uint16_t>(m_BE + 1),
					static_cast<uint16_t>(m_macMaxBE));
			m_NB++;
			if (m_NB > m_macMaxCSMABackoffs) {
				...
				NS_LOG_DEBUG(
						"Perform another backoff; m_NB = " << static_cast<uint16_t> (m_NB));
				m_randomBackoffEvent = Simulator::ScheduleNow(
...
void LrWpanCsmaCa::SetLrWpanMacStateCallback(LrWpanMacStateCallback c) {
	NS_LOG_FUNCTION(this);
	m_lrWpanMacStateCallback = c;
}

int64_t LrWpanCsmaCa::AssignStreams(int64_t stream) {
	NS_LOG_FUNCTION(this);
	m_random->SetStream(stream);
	return 1;
}

uint8_t LrWpanCsmaCa::GetNB(void) {
	return m_NB;
}
