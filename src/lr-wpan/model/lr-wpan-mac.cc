...
TypeId LrWpanMac::GetTypeId(void) {
	static TypeId tid =
			TypeId("ns3::LrWpanMac").SetParent<Object>().SetGroupName("LrWpan").AddAttribute(
					...
					"MacTxEnqueue", "Trace source indicating a packet has been "
					...
					"ns3::Packet::TracedCallback").AddTraceSource("MacRxDrop",
					...
					"ns3::Packet::TracedCallback").AddTraceSource("Sniffer",
					...
					"ns3::Packet::TracedCallback").AddTraceSource("MacState",
					"The state of LrWpan Mac",
					MakeTraceSourceAccessor(&LrWpanMac::m_macStateLogger),
					"ns3::LrWpanMac::StateTracedCallback").AddTraceSource(
					"MacSentPkt",
...
void LrWpanMac::DoDispose() {
	...
	m_txQueue.clear();
	m_phy = 0;
	m_mcpsDataIndicationCallback = MakeNullCallback<void,
...
void LrWpanMac::McpsDataRequest(McpsDataRequestParams params, Ptr<Packet> p) {
	...
	LrWpanMacHeader macHdr(LrWpanMacHeader::LRWPAN_MAC_DATA,
			m_macDsn.GetValue());
	m_macDsn++;
	...
	default:
		NS_LOG_ERROR(
				this << " Can not send packet with incorrect Source Address mode = " << params.m_srcAddrMode);
		confirmParams.m_status = IEEE_802_15_4_INVALID_ADDRESS;
		if (!m_mcpsDataConfirmCallback.IsNull()) {
			m_mcpsDataConfirmCallback(confirmParams);
		}
	...
	if (m_csmaCa->IsSlottedCsmaCa()) {
		...
		if (b1 != 0) {
			NS_LOG_ERROR(
					this << "for non-beacon-enables PAN, bit 1 should always be set to 0");
			confirmParams.m_status = IEEE_802_15_4_INVALID_PARAMETER;
			if (!m_mcpsDataConfirmCallback.IsNull()) {
				m_mcpsDataConfirmCallback(confirmParams);
			}
...
Ptr<LrWpanCsmaCa> LrWpanMac::GetCsmaCa(void) {
	return m_csmaCa;
}
				...
				if (receivedMacHdr.IsData()
					...
					if (receivedMacHdr.GetSeqNum() == macHdr.GetSeqNum()) {
						...
						if (!PrepareRetransmission()) {
							m_setMacState.Cancel();
							m_setMacState = Simulator::ScheduleNow(
									&LrWpanMac::SetLrWpanMacState, this,
									MAC_IDLE);
						} else {
							m_setMacState.Cancel();
							m_setMacState = Simulator::ScheduleNow(
...
void LrWpanMac::AckWaitTimeout(void) {
	NS_LOG_FUNCTION(this);
...
bool LrWpanMac::PrepareRetransmission(void) {
	NS_LOG_FUNCTION(this);
	...
	if (status == IEEE_802_15_4_PHY_SUCCESS) {
		if (!macHdr.IsAcknowledgment()) {
			...
			if (macHdr.IsAckReq()) {
				...
				Time waitTime = MicroSeconds(
						GetMacAckWaitDuration() * 1000 * 1000
								/ m_phy->GetDataOrSymbolRate(false));
				NS_ASSERT(m_ackWaitTimeout.IsExpired());
				m_ackWaitTimeout = Simulator::Schedule(waitTime,
						&LrWpanMac::AckWaitTimeout, this);
				m_setMacState.Cancel();
				m_setMacState = Simulator::ScheduleNow(
						&LrWpanMac::SetLrWpanMacState, this, MAC_ACK_PENDING);
				return;
			} else {
				m_macTxOkTrace(m_txPkt);
				// remove the copy of the packet that was just sent
...
void LrWpanMac::PlmeGetAttributeConfirm(LrWpanPhyEnumeration status,
		LrWpanPibAttributeIdentifier id, LrWpanPhyPibAttributes *attribute) {
...
LrWpanAssociationStatus LrWpanMac::GetAssociationStatus(void) const {
...
uint16_t LrWpanMac::GetPanId(void) const {
	return m_macPanId;
}

void LrWpanMac::SetPanId(uint16_t panId) {
	m_macPanId = panId;
}

void LrWpanMac::ChangeMacState(LrWpanMacState newState) {
	NS_LOG_LOGIC(
			this << " change lrwpan mac state from " << m_lrWpanMacState << " to " << newState);
	m_macStateLogger(m_lrWpanMacState, newState);
	m_lrWpanMacState = newState;
}

uint64_t LrWpanMac::GetMacAckWaitDuration(void) const {

	if (m_phy->IsJrbEnabled()) {
		return m_csmaCa->GetUnitBackoffPeriod() + m_phy->aTurnaroundTime
				+ m_phy->GetPhySHRDuration() + 8 
...
uint8_t LrWpanMac::GetMacMaxFrameRetries(void) const {
...
void LrWpanMac::NotifyRx() {
	NS_LOG_FUNCTION(this);
}

void LrWpanMac::NotifyRxStart() {
	NS_LOG_FUNCTION(this);
}

void LrWpanMac::NotifyTx() {
	NS_LOG_FUNCTION(this);
}

void LrWpanMac::NotifyTxStart() {
	NS_LOG_FUNCTION(this);
}

void LrWpanMac::NotifySleep(void) {
	NS_LOG_FUNCTION(this);
}

void LrWpanMac::NotifyTransition(LrWpanPhyEnumeration nextState) {
	NS_LOG_FUNCTION(this);
}
