...
int main(int argc, char *argv[]) {
	...
	cmd.AddValue("jrbEnabled", "enable the jrb of the phys", jrbEnabled);
	...
	if (jrbEnabled) {
		...
		dev0->GetPhy()->DisableJRB();
		dev1->GetPhy()->DisableJRB();
	}
	//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
//  dev0->GetMac ()->McpsDataRequest (params, p0);
	...
	for (int i = 0; i < 1; i++) { //i was <1000
		// Send a packet back at time 2 seconds
		Ptr<Packet> p2 = Create<Packet>(100);  // 60 bytes of dummy data
		params.m_dstAddr = Mac16Address("00:03");
		Simulator::ScheduleWithContext(2, Seconds(2.0+i),
				&LrWpanMac::McpsDataRequest, dev1->GetMac(), params, p2);
